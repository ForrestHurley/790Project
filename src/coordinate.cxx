...
Coordinate Coordinate::Pow(double power) const
{
    Coordinate out = *this;
    for (int i = 0; i < out.values.size(); i++)
        out.values.at(i) = std::pow(out.values.at(i),power);
    return out;
}

Coordinate Coordinate::Pow(double base, Coordinate exponent)
{
    for (int i = 0; i < exponent.values.size(); i++)
        exponent.values.at(i) = 
            std::pow(base, exponent.values.at(i));
    return exponent;
}

Coordinate Coordinate::FlatTorusDisplacement(
        const Coordinate& a,
        const Coordinate& b,
        const Coordinate& range)
{
  assert(a.dimension == b.dimension);
  assert(a.dimension == range.dimension);
  Coordinate difference = (((b % range) + range) % range) - (((a % range) + range) % range);
  std::vector<double> difference_vector = difference.asVector();
  difference -= range * difference.Sign();

  for (int i = 0; i < a.dimension; i++)
  {
    if (std::abs(difference_vector.at(i)) >
      std::abs(difference.asVector().at(i)))
    {
